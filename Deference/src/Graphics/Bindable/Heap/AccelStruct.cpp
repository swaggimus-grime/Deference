...
TLAS::TLAS(Graphics& g, const D3D12_CPU_DESCRIPTOR_HANDLE& handle, const std::vector<Shared<Drawable>>& drawables)
	:Resource(handle)
{
	...
	ComPtr<ID3D12Resource> instances;
	{
		...
		auto res = CD3DX12_RESOURCE_DESC::Buffer(sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * 1);
	...
	SecureZeroMemory(instanceDescs, sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * drawables.size());
	for (UINT i = 0; i < drawables.size(); i++)
	{
		...
		instanceDescs[i].AccelerationStructure = drawables[i]->BLAS()->GetGPUVirtualAddress();
