...
DiffuseGIPipeline::DiffuseGIPipeline(Graphics& g)
{
	...
	}
	{
		...
		ranges[4].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 4); //Scene
		...
		params[6].InitAsConstantBufferView(0); //light
		params[7].InitAsConstantBufferView(1); //constants
	...
	}
	{
		...
		ranges[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 5); 
