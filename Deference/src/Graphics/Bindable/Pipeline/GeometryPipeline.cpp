...
GeometryPipeline::GeometryPipeline(Graphics& g)
{
    ...
    rootParameters[4].InitAsConstantBufferView(0, 0, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_VOLATILE, D3D12_SHADER_VISIBILITY_VERTEX);

    auto sig = MakeShared<RootSig>(g, _countof(rootParameters), rootParameters);
    Create(g, std::move(sig), vs, ps, std::move(layout), 4);
}
