...
Texture2D::Texture2D(Graphics& g, const std::wstring_view& path)
{
    ComPtr<ID3D12Resource> uptex;
    DirectX::ScratchImage image;
    HR DirectX::LoadFromWICFile(path.data(), DirectX::WIC_FLAGS_IGNORE_SRGB, nullptr, image);
    ...
    {
        const auto transition = CD3DX12_RESOURCE_BARRIER::Transition(m_Res.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        g.CL().ResourceBarrier(1, &transition);
    }
    ...
    HR g.Device().CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&m_DescHeap));
    ...
    g.Device().CreateShaderResourceView(m_Res.Get(), &srvDesc, m_DescHeap->GetCPUDescriptorHandleForHeapStart());
}

void Texture2D::Bind(Graphics& g) const
{
    ID3D12DescriptorHeap* descHeaps[] = { m_DescHeap.Get() };
    g.CL().SetDescriptorHeaps(1, descHeaps);
    g.CL().SetGraphicsRootDescriptorTable(1, m_DescHeap->GetGPUDescriptorHandleForHeapStart());
}
